...
package com.sknop.kafka.connect.transforms;
...
public abstract class Cast<R extends ConnectRecord<R>> implements Transformation<R> {
    private static final Logger log = LoggerFactory.getLogger(com.sknop.kafka.connect.transforms.Cast.class);
    ...
    public static final String OVERVIEW_DOC =
            "Cast fields or the entire key or value to a specific type, e.g. to force an integer field to a smaller "
                    ...
                    + "<p/>Use the concrete transformation type designed for the record key (<code>" + com.sknop.kafka.connect.transforms.Cast.Key.class.getName() + "</code>) "
                    + "or value (<code>" + com.sknop.kafka.connect.transforms.Cast.Value.class.getName() + "</code>).";
    ...
    private static Object castValueToType(Schema schema, Object value, Schema.Type targetType) {
        try {
            ...
            validCastType(inferredType, com.sknop.kafka.connect.transforms.Cast.FieldType.INPUT);
    ...
    private static Map<String, Schema.Type> parseFieldTypes(List<String> mappings) {
        ...
        for (String mapping : mappings) {
            ...
            if (parts.length > 2) {
                // "renames" is a hack to avoid accessing the package private ConfigName.RENAMES
            ...
            if (parts.length == 1) {
                ...
                m.put(WHOLE_VALUE_CAST, validCastType(targetType, com.sknop.kafka.connect.transforms.Cast.FieldType.OUTPUT));
                ...
                m.put(parts[0].trim(), validCastType(type, com.sknop.kafka.connect.transforms.Cast.FieldType.OUTPUT));
            }
        }
    ...
    private static Schema.Type validCastType(Schema.Type type, com.sknop.kafka.connect.transforms.Cast.FieldType fieldType) {
        switch (fieldType) {
            case INPUT:
